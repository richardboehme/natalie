class BindingGen
  ...
  def init
    ...
    @bindings.values.each do |binding|
      ...
      puts "    #{binding.rb_class_as_c_variable}->#{binding.define_method_name}(env, #{binding.rb_method.inspect}_s, #{binding.name}, #{binding.arity}, nullptr, #{binding.optimized ? 'true' : 'false'});"
